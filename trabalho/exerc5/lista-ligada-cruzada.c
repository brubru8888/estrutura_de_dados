#include <stdio.h>
#include <stdlib.h>
#include "lista-ligada-cruzada.h"
...
// Função para liberar a memória alocada para a matriz esparsa
void freeMatrix(SparseMatrix *matriz) {
    // Percorre cada lista de cabeçalhos de linha
    for (int i = 0; i < matriz->num_rows; i++) {
        Node *current = matriz->head_row[i].first;
        while (current != NULL) {
            Node *temp = current;
    ...
    // Libera os arrays de cabeçalhos de linha e coluna
    free(matriz->head_row);
    free(matriz->head_column);
    // Libera a estrutura da matriz esparsa
    free(matriz);
}